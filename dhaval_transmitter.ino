...
#include <nRF24L01.h>
#include <RF24.h>
int roll = 1500, pitch = 1500, yaw = 1500, throttle = 1000, value;
int data[4];
...
void loop() {
  ...
  */
  ...
  value = analogRead(A0);
  throttle = map(value, 0, 1023, 1000, 1350);
  roll = map(Total_angle[0], -60, 60, -175, 175);
  pitch = map(Total_angle[1], -60, 60, -175, 175);
    if(roll <-175)
    {
      roll=-175;
      Total_angle[0]=-60;
